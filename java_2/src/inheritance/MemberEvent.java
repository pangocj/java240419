...
public class MemberEvent extends Member {
	...
	*/
	...
	//메소드 오버라이딩(Method Overriding) : 상속관계의 클래스에서 부모클래스의 메소드를 자식 
	//클래스에서 사용하기 불적절한 경우 부모클래스의 메소드를 자식클래스에서 다시 선언하는 기능
	// => 메소드 오버라이딩 작성 규칙 : 부모클래스의 메소드와 같은 접근 제한자, 반환형, 메소드명 
	//, 매개변수 자료형과 갯수, 예외전달을 자식클래스에서 동일하게 작성 
