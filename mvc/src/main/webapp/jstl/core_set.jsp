...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- taglib 디렉티브를 사용해 Core 태그 라이브러리를 JSP 문서에 포함해야만 Core 태그 라이브러리의
커스텀 태그를 제공받아 사용 가능 --%>
...
<body>
	...
	<%-- set 태그의 value 속성 대신 태그내용을 사용해 Scope 속성값으로 저장될 객체 설정 가능 --%>
	<%-- => Scope 속성값의 속성명이 같은 경우 Scope 속성값을 변경 처리 --%>
	<c:set var="su">20</c:set>
	<p>su = ${su }</p>
	
	<%-- value 속성값으로 EL를 사용해 Scope 속성값을 값(객체)을 제공받아 저장 가능 --%>
	...
	<%-- EL 표현식으로 Scope 속성명의 속성값을 객체로 제공받은 경우 자동으로 toString() 메소드를
