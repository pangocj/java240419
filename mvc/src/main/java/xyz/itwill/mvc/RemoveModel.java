...
//클라이언트가 [/remove.do]로 페이지를 요청한 경우 객체로 생성될 모델 역활의 클래스
// => 아이디를 전달받아 USERINFO 테이블의 행을 삭제하고 [/list.do] 주소로 리다이렉트 이동하기
//위한 정보가 저장된 ActionForward 객체 반환 
// => 관리자가 자신의 정보를 삭제하면 [/logout.do] 주소로 리다이렉트 이동하기 위한 정보가 저장된 
//ActionForward 객체 반환  
// => 관리자만 요청 가능하도록 권한 설정
public class RemoveModel implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			//UserinfoService 클래스의 removeUserinfo() 메소드를 호출하여 회원정보 삭제
