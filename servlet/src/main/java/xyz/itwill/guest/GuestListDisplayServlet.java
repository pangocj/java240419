...
public class GuestListDisplayServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//검색결과(List 객체)를 HTML 문서에 포함하여 응답 처리
		...
		if(guestList.isEmpty()) {//List 객체에 저장된 요소가 없는 경우
			...
			//List 객체에 저장된 모든 요소값(GuestDTO 객체)를 차례대로 제공받아 변수에 저장하는 반복문
			for(GuestDTO guest : guestList) {
				...
				out.println("<th width='150'>작성날짜</th>");
