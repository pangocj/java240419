...
// => [web.xml] 파일의 servlet 엘리먼트와 servlet-mapping 엘러믄토로 제공되는 기능과 동일
//@WebServlet 어노테이션의 속성을 사용하여 서블릿의 이름 설정 및 URL 패턴에 대한 매핑 설정
// => 어노테이션의 속성은 () 기호 안에 작성하며 [,] 기호를 사용해 속성 구분
// => name 속성 : 서블릿의 이름을 속성값으로 설정 - 속성을 생략한 경우 클래스의 이름을 서블릿의
//이름으로 자동 설정
...
public class FirstServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//요청에 대한 처리 명령 작성
		// => 서블릿은 모든 클라이언트에게 일관성 있는 처리결과가 저장된 파일을 동적으로 생성하여 응답
		...
		//SimpleDateFormat 객체에 저장된 패턴의 문자열로 변환하여 반환받아 저장
		String displayNow=dateFormat.format(now);
		
		//처리결과를 파일로 생성하여 응답 처리
