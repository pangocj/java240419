...
// => Service 클래스의 메소드는 DAO 클래스의 객체로 다수의 메소드를 호출하여 기능 구현 - DAO 모듈화
// => Service 클래스의 메소드에서 DAO 클래스의 메소드를 호출하기 위해 Service 클래스에 DAO 클래스의
//객체를 저장할 수 있는 필드 작성 - DAO 클래스와 Service 클래스를 포함관계(의존관계)로 설정
// => Service 클래스가 교체돼도 Service 클래스를 사용하는 클래스(Medel 클래스)에 영향을 최소화 하기
...
public class StudentServiceImpl implements StudentService {
	...
	//저장된 자식클래스 객체의 오버라이드 선언된 메소드 호출 - 오버라이드에 의한 다형성
